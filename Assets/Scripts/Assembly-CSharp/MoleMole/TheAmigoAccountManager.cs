...
namespace MoleMole
{
	public class TheAmigoAccountManager : TheBaseAccountManager
	{
		private const float PRICE_UNIT_RATE = 1f;

		public static bool DebugBuild;

		public TheAmigoAccountManager()
		{
			_accountDelegate = new TheAmigoAccountDelegate();
		}

		public override void Init()
		{
			base.AccountType = (AccountType)13;
		...
		public override bool Pay(string productID, string productName, float productPrice)
		{
			...
			_accountDelegate.pay(productID, productName, productPrice, Guid.NewGuid().ToString("N"), userId.ToString(), oaServerUrl + "/callback/gionee", "AccountEventListener", "PayFinishedCallBack", null);
			return false;
		}
