...
namespace MoleMole
{
	public class MonoMeiCK : MonoMei
	{
		private bool _isInRun;

		private string _attackRunTriggerID = "TriggerRunToAtk";

		private string _attackRunStopSkillID = "ATK01_SP";
		...
		private void SkillIDChangedCallback(string from, string to)
		{
			...
			{
				List<CollisionResult> list = CollisionDetectPattern.CircleCollisionDetectBySphere(base.RootNodePosition, 0f, base.FaceDirection, config.CommonArguments.CollisionRadius * 3f, 1 << InLevelData.MONSTER_LAYER);
				if (list.Count > 0)
				{
		...
		protected override void OnCollisionEnter(Collision collision)
		{
			...
			if (InLevelData.MONSTER_LAYER == collision.gameObject.layer && _isInRun)
			{
				SetTrigger(_attackRunTriggerID);
				_isInRun = false;
			}
