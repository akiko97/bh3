namespace MoleMole.Config
{
	...
	public class ResetAnimatorTrigger : ConfigAbilityAction, IHashable
	{
		...
		{
			HashUtils.ContentHashOnto(TriggerID, ref lastHash);
		...
		{
			output = string.Format("{0} 对 {1} 重置 Animator Trigger {2}", Miscs.GetDebugActorName(instancedAbility.caster), Miscs.GetDebugActorName(target), TriggerID);
			return true;
		}
	}
}
