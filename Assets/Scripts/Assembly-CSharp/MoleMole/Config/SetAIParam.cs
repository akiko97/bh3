namespace MoleMole.Config
{
	...
	public class SetAIParam : ConfigAbilityAction, IHashable
	{
		...
		public void ObjectContentHashOnto(ref int lastHash)
		{
			...
			HashUtils.ContentHashOnto(IsBool, ref lastHash);
			HashUtils.ContentHashOnto((int)LogicType, ref lastHash);
		...
		{
			output = string.Format("{0} 对 {1} 设置 AI 参数 {2}:{3}:{4}", Miscs.GetDebugActorName(instancedAbility.caster), Miscs.GetDebugActorName(target), Param, Value, LogicType);
			return true;
		}
	}
}
