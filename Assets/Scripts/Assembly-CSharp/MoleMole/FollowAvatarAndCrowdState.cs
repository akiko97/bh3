...
namespace MoleMole
{
	public class FollowAvatarAndCrowdState : FollowAvatarAndTargetState
	{
		...
		public FollowAvatarAndCrowdState(MainCameraFollowState followState)
			: base(followState)
		{
		...
		protected override void PreCalculate()
		{
			...
			if (attackTarget != null)
			{
				...
				{
					baseMonoEntity = null;
					vector = GetCrowdPosition();
				}
				else
				{
					baseMonoEntity = attackTarget;
			...
			{
				_targetCollisionRadius = GetColliderRadius(baseMonoEntity);
			}
			else
			{
				_targetCollisionRadius = 0f;
			}
		}
		...
		private void SampleBossTargetPosition()
		{
			...
			for (int i = 0; i < allMonsters.Count; i++)
			{
				...
				vector += baseMonoMonster.XZPosition;
			}
			Vector3 vector2 = vector / allMonsters.Count;
			for (int num = _followPositions.Length - 1; num > 0; num--)
			{
				_followPositions[num] = _followPositions[num - 1];
			}
			_followPositions[0] = vector2;
		}
		...
		private Vector3 GetCrowdPosition()
		{
			...
			return zero / _followPositions.Length;
		}
	}
}
