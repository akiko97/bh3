...
namespace PigeonCoopToolkit.Effects.Trails
{
	public abstract class TrailRenderer_Base : MonoBehaviour
	{
		public class MaterialsContainer
		{
			...
			{
				MaterialsContainer materialsContainer = obj as MaterialsContainer;
				...
				if (materials == null)
				{
					return materialsContainer.materials == null;
				}
				if (materials.Length != materialsContainer.materials.Length)
				{
					return false;
				}
				for (int i = 0; i < materials.Length; i++)
				{
					if (materials[i] != materialsContainer.materials[i])
					{
						return false;
					}
				}
		...
		public float StretchUpRatio = 1f;

		public float StretchDownRatio = 1f;

		public bool clockSize;
		...
		protected static Dictionary<MaterialsContainer, List<PCTrail>> _matToTrailList;

		protected static List<Mesh> _toClean;

		protected static bool _hasRenderer;

		protected static int GlobalTrailRendererCount;
		...
		protected virtual void Awake()
		{
			...
			{
				_matToTrailList = new Dictionary<MaterialsContainer, List<PCTrail>>();
		...
		protected virtual void Start()
		{
			if (clockSize)
			{
				float stretchDownRatio = StretchDownRatio;
				StretchDownRatio = StretchUpRatio;
				StretchUpRatio = stretchDownRatio;
			}
		}
		...
		protected virtual void LateUpdate()
		{
			...
			_hasRenderer = true;
			Dictionary<MaterialsContainer, List<PCTrail>>.Enumerator enumerator = _matToTrailList.GetEnumerator();
			try
			{
				while (enumerator.MoveNext())
				{
					List<PCTrail> value = enumerator.Current.Value;
					...
					_toClean.Add(mesh);
					MaterialsContainer key = enumerator.Current.Key;
					for (int j = 0; j < key.materials.Length; j++)
					{
						DrawMesh(mesh, key.materials[j]);
					}
					value.Clear();
				}
			}
		...
		protected virtual void Update()
		{
			...
			if (!_matToTrailList.ContainsKey(TrailData.GetMaterialsContainer()))
			{
				_matToTrailList.Add(TrailData.GetMaterialsContainer(), new List<PCTrail>());
			}
			if (_activeTrail != null)
			{
				...
				_matToTrailList[TrailData.GetMaterialsContainer()].Add(_activeTrail);
			}
			for (int num = _fadingTrails.Count - 1; num >= 0; num--)
			{
				...
				else
				{
					...
					_matToTrailList[TrailData.GetMaterialsContainer()].Add(_fadingTrails[num]);
				}
			}
			CheckEmitChange();
		}
		...
		private void GenerateMesh(PCTrail trail)
		{
			...
			for (int i = 0; i < trail.Points.Count; i++)
			{
				...
				trail.verticies[num] = pCTrailPoint.Position + zero * num3 * StretchUpRatio;
				if (clockSize)
				{
				...
				else if (TrailData.MaterialTileLength <= 0f)
				{
				...
				num++;
				trail.verticies[num] = pCTrailPoint.Position - zero * num3 * StretchDownRatio;
				if (clockSize)
				{
					if (TrailData.MaterialTileLength <= 0f)
					{
		...
		protected virtual void UpdatePoints(PCTrail line, float deltaTime)
		{
